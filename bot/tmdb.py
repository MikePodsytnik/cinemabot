...
async def _search_multi(
    ...
    async with session.get(f"{_TMBD_BASE}/search/multi", params=params) as resp:
...
async def tmdb_search_best(
    ...
    return _pick_first_movie_tv(en, query)
...
async def tmdb_fetch_details(
    ...
    async with session.get(f"{_TMBD_BASE}/{media_type}/{tmdb_id}", params=params) as resp:
        if resp.status >= 400:
            return None
